...
class TestApp(unittest.TestCase):
    ...
    def test_react_change_password(self):
        ...
        self.assertEqual(response_dict.get('error', None), 'Confirming password not supplied')
        response = self.client.post('/api/change_password', headers={'Authorization': auth_header}, json={'username': 'chardin', 'current_password': 'foo', 'new_password': 'bar', 'confirm_password': 'baz'})
        response_dict = json.loads(response.data)
        self.assertFalse(response_dict.get('success', True), False)
        self.assertEqual(response_dict.get('error', None), 'Confirming password does not match new password')
        response = self.client.post('/api/change_password', headers={'Authorization': auth_header}, json={'username': 'chardin', 'current_password': '', 'new_password': 'bar'})
        response_dict = json.loads(response.data)
        self.assertFalse(response_dict.get('success', True), False)
        self.assertEqual(response_dict.get('error', None), 'Current password not supplied')
        response = self.client.post('/api/change_password', headers={'Authorization': auth_header}, json={'username': 'chardin', 'current_password': 'baz', 'new_password': ''})
        response_dict = json.loads(response.data)
        self.assertFalse(response_dict.get('success', True), False)
        self.assertEqual(response_dict.get('error', None), 'New password not supplied')
        response = self.client.post('/api/change_password', headers={'Authorization': auth_header}, json={'username': 'chardin', 'current_password': 'baz', 'new_password': 'foo', 'confirm_password': 'foo'})
        response_dict = json.loads(response.data)
